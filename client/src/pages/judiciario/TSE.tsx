...
import { buscarTSE } from "../../services/politicaService";
...
export default function TSE() {
  ...
  useEffect(() => {
    async function carregar() {
      setLoading(true);
      try {
        setLista((await buscarTSE()) || []);
      } catch (e) {
        console.error("Erro ao carregar TSE:", e);
        setLista([]);
      } finally {
        setLoading(false);
      }
      ...
      <div className="px-4 md:px-8 pt-6 max-w-6xl mx-auto">
        ...
        <div className="flex gap-1.5 md:gap-2 overflow-x-auto pb-1 mb-6 -mx-4 px-4 md:mx-0 md:px-0">
          {[
            ...
            { to: "/judiciario/tse", label: "TSE", active: true },
        ...
        <div className="space-y-6">
            ...
            {competenciasAbertas && (
              <div className="mt-4 pt-4 border-t border-slate-100">
                <p className="text-xs md:text-sm text-slate-600 mb-4">
                  O Tribunal Superior Eleitoral é o órgão máximo da Justiça Eleitoral brasileira.
                </p>
          ...
          {presidente && (
            <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-6">
              <h2 className="text-sm md:text-base font-bold text-slate-800 mb-3">Presidente do TSE</h2>
          ...
          <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-5">
              ...
              <input type="text" placeholder="Buscar ministro ou cargo..." value={busca} onChange={(e) => setBusca(e.target.value)}
            ...
            {loading ? (
              ...
              <div className="border border-dashed rounded-xl p-8 text-center text-slate-400 text-sm">
                Nenhum registro. Cadastre no Admin → Judiciário (órgão TSE).
              </div>
            ) : (
                  ...
                  <div key={p.id} className="border border-slate-200 rounded-xl overflow-hidden">
                    ...
                    {abertoId === p.id && (
                      <div className="px-4 pb-4 border-t bg-slate-50 text-xs md:text-sm text-slate-700 space-y-1.5">
                        ...
                        {!p.indicado_por && !p.data_inicio && !p.origem && (
