...
import { buscarTST } from "../../services/politicaService";
...
export default function TST() {
  ...
  useEffect(() => {
    async function carregar() {
      setLoading(true);
      try {
        setLista((await buscarTST()) || []);
      } catch (e) {
        console.error("Erro ao carregar TST:", e);
        setLista([]);
      } finally {
        setLoading(false);
      }
      ...
      <div className="px-4 md:px-8 pt-6 max-w-6xl mx-auto">
        ...
        <div className="flex gap-1.5 md:gap-2 overflow-x-auto pb-1 mb-6 -mx-4 px-4 md:mx-0 md:px-0">
          {[
            ...
            { to: "/judiciario/tst", label: "TST", active: true },
        ...
        <div className="space-y-6">
          ...
          {presidente && (
            <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-6">
              <h2 className="text-sm md:text-base font-bold text-slate-800 mb-3">Presidente do TST</h2>
          ...
          <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-6">
            ...
            {loading ? (
              ...
              <div className="border border-dashed rounded-xl p-8 text-center text-slate-400 text-sm">
                Nenhum registro. Cadastre no Admin → Judiciário (órgão TST).
              </div>
            ) : (
