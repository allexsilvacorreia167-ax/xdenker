...
export default function Camara() {
  ...
  const deputadosFiltrados = deputados.filter((p) => {
    ...
    const matchUf = filtroUf === "Todos os estados" || (p.uf || "").toUpperCase() === filtroUf.toUpperCase();
    const matchEspectro = filtroEspectro === "Todos os espectros" || (p.espectro || "").toLowerCase() === filtroEspectro.toLowerCase();
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20">
      <div className="px-4 md:px-8 pt-6 max-w-6xl mx-auto">
        <Link to="/sistema-politico" className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 mb-4">
          ← Voltar à Visão Geral
        </Link>
        <h1 className="text-base md:text-lg font-bold text-slate-800 mb-1">Câmara dos Deputados</h1>
        <p className="text-xs md:text-sm text-slate-500 mb-6">Composição e deputados federais da câmara baixa</p>
        ...
        <div className="space-y-6">
          <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 text-center">
              <div className="text-2xl md:text-3xl font-bold text-slate-800">{deputados.length || 513}</div>
          ...
          <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs md:text-sm text-slate-500">{deputados.length || 513} cadeiras</span>
            </div>
            <div className="bg-slate-50 border border-slate-200 rounded-xl h-64 flex items-end justify-center pb-8">
              <div className="text-center">
                <div className="text-4xl md:text-5xl font-bold text-slate-300">{deputados.length || 513}</div>
                <div className="text-xs text-slate-400 mt-2">Hemiciclo da Câmara dos Deputados</div>
              </div>
            </div>
          </section>
          ...
          <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-5">
              ...
              <div className="flex flex-wrap gap-2 w-full md:w-auto">
                <input type="text" placeholder="Buscar deputado, partido..." value={busca} onChange={(e) => setBusca(e.target.value)}
                ...
                <select value={filtroUf} onChange={(e) => setFiltroUf(e.target.value)} className="border border-slate-200 rounded-xl px-3 py-1.5 text-xs md:text-sm bg-slate-50">
                  <option>Todos os estados</option>
                  {["AC", "AL", "AP", "AM", "BA", "CE", "DF", "ES", "GO", "MA", "MT", "MS", "MG", "PA", "PB", "PR", "PE", "PI", "RJ", "RN", "RS", "RO", "RR", "SC", "SP", "SE", "TO"].map(uf => (
                    <option key={uf}>{uf}</option>
                  ))}
                </select>
                <select value={filtroEspectro} onChange={(e) => setFiltroEspectro(e.target.value)} className="border border-slate-200 rounded-xl px-3 py-1.5 text-xs md:text-sm bg-slate-50">
                  <option>Todos os espectros</option>
                  <option>Esquerda</option><option>Centro-Esquerda</option><option>Centro</option><option>Centro-Direita</option><option>Direita</option>
                </select>
              </div>
            </div>

            <div className="text-xs md:text-sm text-slate-500 mb-4">Mostrando {deputadosFiltrados.length} de {deputados.length} deputados.</div>

            {loading ? (
              <div className="border border-slate-200 rounded-xl p-8 text-center text-slate-400 text-sm">Carregando deputados...</div>
              ...
              <div className="border border-dashed border-slate-200 rounded-xl p-8 text-center text-slate-400 text-sm">Nenhum deputado encontrado com os filtros selecionados.</div>
            )}
          </section>
        </div>
      </div>
