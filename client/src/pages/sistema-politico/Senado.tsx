...
import { buscarSenadores, buscarSuplentesDoTitular } from "../../services/politicaService";
...
export default function Senado() {
  ...
  useEffect(() => {
    async function carregar() {
      try {
        const dados = await buscarSenadores();
        setSenadores(dados || []);
      } catch (error) {
  ...
  const senadoresFiltrados = senadores.filter((p) => {
    const matchTexto =
    ...
    return matchTexto && matchEspectro;
  });
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20">
      <div className="px-4 md:px-8 pt-6 max-w-6xl mx-auto">
        ...
        <h1 className="text-base md:text-lg font-bold text-slate-800 mb-1">Senado Federal</h1>
        <p className="text-xs md:text-sm text-slate-500 mb-6">Composição e senadores do parlamento federal</p>
        ...
        <div className="flex gap-1.5 md:gap-2 overflow-x-auto pb-1 mb-6 -mx-4 px-4 md:mx-0 md:px-0">
          {[
            ...
            { to: "/sistema-politico/senado", label: "Senado", active: true },
        ...
        <div className="space-y-6">
          <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 text-center">
              <div className="text-2xl md:text-3xl font-bold text-slate-800">{senadores.length || 81}</div>
          ...
          <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs md:text-sm text-slate-500">{senadores.length || 81} cadeiras</span>
            </div>
            <div className="bg-slate-50 border border-slate-200 rounded-xl h-64 flex items-end justify-center pb-8">
              <div className="text-center">
                <div className="text-4xl md:text-5xl font-bold text-slate-300">{senadores.length || 81}</div>
                <div className="text-xs text-slate-400 mt-2">Hemiciclo do Senado Federal</div>
              </div>
            </div>
          </section>
          ...
          <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-6">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-5">
              ...
              <div className="flex flex-wrap gap-2 w-full md:w-auto">
                <input type="text" placeholder="Buscar senador, partido ou UF..." value={busca} onChange={(e) => setBusca(e.target.value)}
            ...
            {loading ? (
              <div className="border border-slate-200 rounded-xl p-8 text-center text-slate-400 text-sm">Carregando senadores...</div>
            ) : senadoresFiltrados.length > 0 ? (
              <div className="space-y-2">
                {senadoresFiltrados.map((p) => (
                  <div key={p.id} className="border border-slate-200 rounded-xl overflow-hidden">
                    <button onClick={() => toggleSuplentes(p.id)} className="w-full flex items-center gap-3 p-3 hover:bg-slate-50 transition-colors text-left">
                      <div className="w-10 h-10 rounded-full bg-slate-200 flex-shrink-0 flex items-center justify-center text-xs font-bold text-slate-600">{(p.nome || "S").charAt(0)}</div>
              ...
              <div className="border border-dashed border-slate-200 rounded-xl p-8 text-center text-slate-400 text-sm">Nenhum senador encontrado com os filtros selecionados.</div>
            )}
          </section>
        </div>
      </div>
