...
Uso:
  set SUPABASE_SERVICE_KEY=sua_chave_aqui
  python importar.py
"""
...
# ============================================================
# CONFIGURAÇÃO (sem secret no código)
# ============================================================
...
if not SUPABASE_SERVICE_KEY:
    raise Exception(
        "Defina a variável de ambiente SUPABASE_SERVICE_KEY.\n"
        "Windows (PowerShell): $env:SUPABASE_SERVICE_KEY='sua_chave'\n"
        "Windows (CMD): set SUPABASE_SERVICE_KEY=sua_chave"
    )

supabase = create_client(SUPABASE_URL, SUPABASE_SERVICE_KEY)

# Pasta dos CSVs (relativa a onde o script roda)
# Se rodar de dentro de etl_tse/: use "dados"
# Se rodar da raiz do projeto: use "etl_tse/dados"
PASTA_DADOS = "dados"
if not os.path.exists(PASTA_DADOS) and os.path.exists("etl_tse/dados"):
    PASTA_DADOS = "etl_tse/dados"
...
def processar_arquivos():
    ...
    for arquivo in arquivos:
        ...
        for _, row in df.iterrows():
            ...
            if cargo not in CARGOS_TITULARES and cargo != "DEPUTADO DISTRITAL":
                continue
            ...
            for i in range(0, len(titulares_lote), 200):
                ...
                try:
                    ...
                    for reg, inserted in zip(lote, resp.data):
                        chave = (reg["uf"], reg["cargo"], reg["numero_urna"])
    ...
    print("======================================")
    print("Lembrete: Presidente, Vice e Ministros → tabela 'executivo' (Admin).")
